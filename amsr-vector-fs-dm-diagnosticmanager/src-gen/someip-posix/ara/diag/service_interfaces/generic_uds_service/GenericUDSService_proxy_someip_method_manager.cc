 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  GenericUDSService_proxy_someip_method_manager.cc
...
GenericUDSServiceProxySomeIpMethodManagerService::GenericUDSServiceProxySomeIpMethodManagerService(
    GenericUDSServiceProxySomeIpBinding& proxy_binding)
    : next_session_id_(0U), proxy_binding_(proxy_binding) {}

ara::com::Future<proxy::methods::Service::Output> GenericUDSServiceProxySomeIpMethodManagerService::HandleMethodRequest(
    uint8& SID, DataArrayType& RequestData, MetaInfoType& MetaInfo) {
  ...
  header.service_id_ = GenericUDSServiceProxySomeIpBinding::kServiceId;
  ...
  // Serialize payload
  /* ImplementationDataTypeValue serialization for element 'SID' (/AUTOSAR_Platform/ImplementationDataTypes/uint8) */
  {
    marshaller::Serializer<marshaller::BEPayloadNoLengthFieldPolicy, decltype(serializer)> serializer_SID{&serializer};
    serializer_SID.PushBack(SID);
    serializer_SID.Close();
  }
  /* ImplementationDataTypeVector serialization for element 'RequestData' (/ara/diag/types/DataArrayType) */
  {
    marshaller::ComplexDataTypeSerializer<marshaller::BEPayloadUint32LengthFieldPolicy, decltype(serializer)>
        serializer_RequestData{&serializer};
    for (const auto& RequestData_element : RequestData) {
      /* ImplementationDataTypeValue serialization for element 'byte' (/ara/diag/types/DataArrayType/byte) */
      {
        marshaller::Serializer<marshaller::BEPayloadNoLengthFieldPolicy, decltype(serializer_RequestData)>
            serializer_RequestData_byte{&serializer_RequestData};
        serializer_RequestData_byte.PushBack(RequestData_element);
        serializer_RequestData_byte.Close();
      }
    }
    serializer_RequestData.Close();
  }
...
class DeserializeGenericUDSServiceServiceResponseData {
 public:
  explicit DeserializeGenericUDSServiceServiceResponseData(RootDeserializer* root) : deserializer_{root} {}
  ...
  std::pair<uint8, bool> DeserializeResponseData_byte(ParentDeserializer* parent) {
    namespace marshaller = someip_posix_common::someip::serialization;
    bool deserialization_ok{true};
    uint8 ResponseData_byte{};
    marshaller::Deserializer<marshaller::BEPayloadNoLengthFieldPolicy, ParentDeserializer>
        deserializer__ResponseData_byte{parent};
    deserialization_ok = deserializer__ResponseData_byte.PopFront(ResponseData_byte) > 0U;
    return std::make_pair(ResponseData_byte, deserialization_ok);
  }

  /* ImplementationDataTypeVector deserialization for element 'ResponseData' (/ara/diag/types/DataArrayType) */
  template <typename ParentDeserializer>
  std::pair<DataArrayType, bool> DeserializeResponseData(ParentDeserializer* parent) {
    namespace marshaller = someip_posix_common::someip::serialization;
    bool deserialization_ok{true};
    marshaller::ComplexDataTypeDeserializer<marshaller::BEPayloadUint32LengthFieldPolicy, ParentDeserializer>
        deserializer__ResponseData{parent};
    DataArrayType ResponseData{};
    while (deserializer__ResponseData.HasDataRemaining()) {
      const auto ret = DeserializeResponseData_byte(&deserializer__ResponseData);
      deserialization_ok &= ret.second;
      if (!deserialization_ok) {
        break;
      }

      ResponseData.push_back(ret.first);
    }
    return std::make_pair(ResponseData, deserialization_ok);
  }

  DataArrayType StartDeserialization() {
    auto ret = DeserializeResponseData<RootDeserializer>(deserializer_);
    return ret.first;
  }
...
template <typename RootDeserializer>
class DeserializeGenericUDSServiceServiceerrorContext {
 public:
  explicit DeserializeGenericUDSServiceServiceerrorContext(RootDeserializer* root) : deserializer_{root} {}
...
void GenericUDSServiceProxySomeIpMethodManagerService::HandleMethodResponse(
  ...
  if (message_type == someip_posix_common::someip::SomeIpMessageType::kResponse &&
    ...
    switch (error_code) {
      case proxy::application_errors::UDSServiceFailed::kErrorCode:
        HandleMethodResponseUDSServiceFailed(std::move(deserializer), std::move(p));
        break;
      default:
    ...
    p.set_exception(std::make_exception_ptr(ara::com::ServiceNotAvailableException("GenericUDSService")));
  } else {
...
template <typename RootDeserializer>
void GenericUDSServiceProxySomeIpMethodManagerService::HandleMethodResponseOk(RootDeserializer&& deserializer,
                                                                              Promise&& p) {
  proxy::methods::Service::Output ret{};
  // Deserialize method return value(s)
  // Create one deserializer object for argument ResponseData
  DeserializeGenericUDSServiceServiceResponseData<typename std::decay<RootDeserializer>::type>
      arg_ResponseData_deserializer{&deserializer};
  DataArrayType ResponseData = arg_ResponseData_deserializer.StartDeserialization();

  ret.ResponseData = ResponseData;
...
template <typename RootDeserializer>
void GenericUDSServiceProxySomeIpMethodManagerService::HandleMethodResponseUDSServiceFailed(
  ...
  DeserializeGenericUDSServiceServiceerrorContext<typename std::decay<RootDeserializer>::type>
  ...
  p.set_exception(std::make_exception_ptr(proxy::application_errors::UDSServiceFailed{errorContext}));
}
