 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  DM_IPC_proxy_someip_binding.cc
...
constexpr someip_posix_common::someip::ServiceId DM_IPCProxySomeIpBinding::kServiceId;
constexpr someip_posix_common::someip::MajorVersion DM_IPCProxySomeIpBinding::kMajorVersion;

DM_IPCProxySomeIpBinding::DM_IPCProxySomeIpBinding(someip_posix_common::someip::InstanceId instance_id,
    ...
    : instance_id_(instance_id),
      ...
      logger_(ara::log::CreateLogger("DM_IPCProxySomeIpBinding", "")),
      method_manager_RequestData_(*this) {
  // Register this proxy binding for method responses and event notifications.
  someip_binding_.RegisterProxyBinding(std::make_tuple(DM_IPCProxySomeIpBinding::kServiceId, instance_id_, client_id_),
                                       this);
}

DM_IPCProxySomeIpBinding::~DM_IPCProxySomeIpBinding() {
  someip_binding_.DeRegisterProxyBinding(
      std::make_tuple(DM_IPCProxySomeIpBinding::kServiceId, instance_id_, client_id_));
}

void DM_IPCProxySomeIpBinding::SendMethodRequest(someip_posix_common::someip::SomeIpPacket&& packet) {
  someip_binding_.SendMethodRequest(instance_id_, std::move(packet));
}

void DM_IPCProxySomeIpBinding::SubscribeEvent(someip_posix_common::someip::EventId event_id,
  ...
  if (insert_pair.second) {
    someip_binding_.SubscribeEvent(DM_IPCProxySomeIpBinding::kServiceId, instance_id_, event_id);
  } else {
    logger_.LogWarn() << "Event with ID " << event_id << " already subscribed! Subscription is dropped.";
  }
}

void DM_IPCProxySomeIpBinding::UnsubscribeEvent(someip_posix_common::someip::EventId event_id) {
  ara::com::someip_posix::ServiceProxySomeIpEventInterface* event_manager = nullptr;
  {
    ...
    if (it != event_subscriptions_.end()) {
      ...
      someip_binding_.UnsubscribeEvent(DM_IPCProxySomeIpBinding::kServiceId, instance_id_, event_id);
    }
  }
...
void DM_IPCProxySomeIpBinding::HandleMethodResponse(const ::someip_posix_common::someip::SomeIpMessageHeader& header,
  ...
  switch (method_id) {
    case DM_IPCProxySomeIpMethodManagerRequestData::kMethodId: {
      method_manager_RequestData_.HandleMethodResponse(header, std::move(deserializer));
      break;
    }
    default: {
...
void DM_IPCProxySomeIpBinding::HandleEventNotification(const ::someip_posix_common::someip::SomeIpMessageHeader& header,
                                                       RootDeserializerAlias&& deserializer) {
...
void DM_IPCProxySomeIpBinding::HandleEventSubscriptionStateUpdate(const someip_posix_common::someip::MethodId event_id,
