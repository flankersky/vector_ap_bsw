 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  write_did_service_handler.cc
 *        \brief  TODO
 *
 *      \details  TODO
 *
...
WriteDidServiceHandler::WriteDidServiceHandler(const Configuration config) {
  processor_factory_.reserve(config.base.max_number_of_service_processors);
}

processor::ServiceProcessor::Ptr WriteDidServiceHandler::CreateServiceProcessor(
  ...
  assert(message->GetPayload()[0] == processor::WriteDidProcessor::kWriteDidSid);
