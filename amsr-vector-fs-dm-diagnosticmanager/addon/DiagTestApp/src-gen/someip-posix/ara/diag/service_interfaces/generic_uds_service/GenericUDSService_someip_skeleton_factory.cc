 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  GenericUDSService_someip_skeleton_factory.cc
...
GenericUDSServiceSomeIpSkeletonFactory::GenericUDSServiceSomeIpSkeletonFactory(
    ara::com::someip_posix::AraComSomeIpBindingInterface& someip_binding)
    : someip_binding_(someip_binding) {}

std::unique_ptr<ara::com::internal::SkeletonImplInterface> GenericUDSServiceSomeIpSkeletonFactory::create(
    ::someip_posix_common::someip::InstanceId instance_id, ara::com::internal::SkeletonInterface& skeleton) const {
  // Downcast the SkeletonInterface to the concrete GenericUDSServiceSkeleton
  // TODO(visswa) Provide a version without dynamic_cast.
  GenericUDSServiceSkeleton* concrete = dynamic_cast<GenericUDSServiceSkeleton*>(&skeleton);
  // If downcasting fails, the factory created the wrong binding-specific skeleton-side.
  assert(concrete != nullptr);

  return vac::language::make_unique<GenericUDSServiceSkeletonSomeIpBinding>(instance_id, someip_binding_, *concrete);
}
