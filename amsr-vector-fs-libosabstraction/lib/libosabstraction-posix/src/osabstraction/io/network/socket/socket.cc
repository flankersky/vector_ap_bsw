...
#include "osabstraction/io/network/address/ifaddrs_ptr.h"
...
Socket::Socket(address::SocketAddressFamily family, SocketType type) : type_{type} {
  int ret = ::socket(address::SocketAddress::toUnixSocketAddressFamily(family), Socket::toUnixSocketType(type), 0);
  if (ret < 0) {
    throw std::system_error(errno, std::generic_category());
  }
  handle_ = ret;
...
ssize_t Socket::Peek(void* buffer, std::size_t size, address::SocketAddress* remote_address) {
  return Receive(buffer, size, remote_address, MSG_PEEK);
}

ssize_t Socket::Receive(void* buffer, std::size_t size, address::SocketAddress* remote_address) {
  return Receive(buffer, size, remote_address, 0);
}

ssize_t Socket::Receive(void* buffer, std::size_t size, address::SocketAddress* remote_address, int flags) {
  struct sockaddr_storage sockaddr;
  socklen_t sockaddr_size = sizeof(sockaddr);
...
std::pair<bool, unsigned int> Socket::InterfaceAddressToIndex(const std::string& address) {
  ...
  auto ifa_ptr = osabstraction::io::network::address::make_ifaddrs_ptr(ifa);
