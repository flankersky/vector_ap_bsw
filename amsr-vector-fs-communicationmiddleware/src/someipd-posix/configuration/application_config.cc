...
ConfigurationTypesAndDefs::ServiceContainer ApplicationConfig::ReadServices() const {
  ...
  //std::cout<<config.HasMember("services")<<std::endl;
  //std::cout<<config["services"].IsArray()<<std::endl;
  ...
  printf("ReadServices 3.1\n");
  printf("ReadServices 3.2\n");
  //std::cout<<config["services"].GetType()<<std::endl;
  ...
  rapidjson::Document document;
   ...
   char buffer[sizeof(json)];
    ...
    if (document.ParseInsitu(buffer).HasParseError())
        return services;
  //document.Parse("/opt/amsrVectorAppExampleCalculatorServer/etc/application-gen.json");
   
    std::ifstream ifs("/opt/amsrVectorAppExampleCalculatorServer/etc/application-gen.json");
    rapidjson::IStreamWrapper isw(ifs);
    document.ParseStream(isw);
    
  const rapidjson::Value& services1 = document["services"];
  std::cout<< "service size is "  <<services1.Size()<<std::endl;
   #endif
